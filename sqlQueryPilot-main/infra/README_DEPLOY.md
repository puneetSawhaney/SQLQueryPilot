# sqlquerypilot Deployment Guide

Complete guide to deploying the sqlquerypilot system on AWS EC2 (Ubuntu 22.04).
...
git clone https://github.com/your-org/sqlquerypilot.git /opt/sqlquerypilot
cd /opt/sqlquerypilot
```
...
scp -i ~/.ssh/your-key.pem \
    ...
    ubuntu@<PUBLIC_IP>:/opt/sqlquerypilot/data/raw/
```
...
```bash
sudo cp infra/sqlquerypilot.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable sqlquerypilot    # Start on boot
sudo systemctl start sqlquerypilot
```

Check it's running:
```bash
sudo systemctl status sqlquerypilot
sudo journalctl -u sqlquerypilot -f    # Follow logs
```
...
```bash
sudo cp infra/nginx.conf /etc/nginx/sites-available/sqlquerypilot
sudo ln -sf /etc/nginx/sites-available/sqlquerypilot /etc/nginx/sites-enabled/sqlquerypilot
...
Expected output:
```
✅ sqlquerypilot.service is ACTIVE
...
sudo systemctl restart sqlquerypilot
```
...
sudo journalctl -u sqlquerypilot -f
