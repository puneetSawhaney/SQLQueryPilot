...
1. **gunicorn not running:**
   ...
   sudo journalctl -u sqlquerypilot -n 50 --no-pager
   sudo systemctl restart sqlquerypilot
   ```
2. **Wrong bind address in service file:** Confirm `--bind 127.0.0.1:8000` in `sqlquerypilot.service`.
...
3. **Permission issue:** Ensure `ubuntu` user owns `chroma_store/`:
   ```bash
   chown -R ubuntu:ubuntu /opt/sqlquerypilot/chroma_store
   ```
...
1. **OPENAI_API_KEY not set or wrong:** Check `.env` file:
   ```bash
   grep OPENAI_API_KEY /opt/sqlquerypilot/.env
   ```
2. **Key has been revoked:** Generate a new key at https://platform.openai.com/api-keys.
3. **EnvironmentFile not loaded:** Confirm `EnvironmentFile=/opt/sqlquerypilot/.env` in the service file and restart.
...
1. **LLM call taking too long:** GPT-4o with a long prompt can take 30-60s. Increase timeouts:
   - `sqlquerypilot.service`: change `--timeout 120` to `--timeout 180`
...
2. **nginx root path wrong:** Confirm `root /opt/sqlquerypilot/frontend/dist;` in nginx.conf.
...
5. **Old dist/ cached:** Hard reload with `Ctrl+Shift+R` or `Cmd+Shift+R`.
   ```bash
   # Rebuild frontend
   cd /opt/sqlquerypilot/frontend && npm run build
