...
APP_DIR="${APP_DIR:-/opt/sqlquerypilot}"
PUBLIC_IP=$(curl -sf --connect-timeout 2 --max-time 5 http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || echo "unknown")

echo "================================================="
echo "  sqlquerypilot Deployment Verification"
echo "  Public IP: $PUBLIC_IP"
echo "================================================="

echo ""
echo "--- [1/5] Checking systemd service status ---"
if systemctl is-active --quiet sqlquerypilot; then
    echo "✅ sqlquerypilot.service is ACTIVE"
else
    echo "❌ sqlquerypilot.service is NOT active"
    journalctl -u sqlquerypilot --no-pager -n 30  # Print last 30 log lines for debugging
    exit 1
fi
